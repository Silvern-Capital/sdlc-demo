...
- `.claude/hooks/qa-review.js` — Stop: the QA review as a hook. When Claude says it is done and the tests are green, it starts a separate `claude -p --agent qa` run on the diff, once per diff, and a blocking finding goes back to Claude before it can stop. Off by default because each run is a billed session of about a minute; turn it on for a session with `BEACON_QA_STOP=1 claude` (`BEACON_QA_STOP=dry` only prints what it would run).
- `scripts/qa-query.js` — the fixed queries the qa agent uses when it runs from the Stop hook, where it has no interpreter: status, diff, sites, api, csv over any input, bytes, tests.
